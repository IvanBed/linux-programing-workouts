...
void read_linear_system(std::vector<std::vector<double>> &matrix)
{
    for (auto & row : matrix)
	{
		for (auto & el : row)
...
std::vector<std::vector<double>> make_coeff_matrix(std::vector<std::vector<double>> const & matrix)
{
...
int main() 
{
	...
	vector<vector<double>> coeff_matrix = make_coeff_matrix(matrix);
	
	res = solve_linear_system(coeff_matrix, solution);
