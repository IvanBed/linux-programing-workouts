...
uint8_t top_int(vector *inst, uint8_t multithread_flag)
{
    size_t type_size = sizeof(int64_t);
    char    num_buff[type_size];
    int64_t el;
    ...
    if (!multithread_flag)
        res = top(inst, type_size, num_buff);
    else
        res = top_multithread(inst, type_size, num_buff);
...
uint8_t pop_int(vector *inst, uint8_t multithread_flag)
{
    size_t type_size = sizeof(int64_t);
    char    num_buff[type_size];
    int64_t el;
    ...
    if (!multithread_flag)
        res = pop(inst, type_size, num_buff);
    else
        res = pop_multithread(inst, type_size, num_buff);
