...
static std::string raw_bytes_to_utf8_internal(std::vector<uint8_t> const & bytes, charset_offset const & offset)
{
    std::string utf8_str = "";
	uint8_t byte_1;
	uint8_t byte_2;	
...
std::string raw_bytes_to_utf8(std::vector<uint8_t> const & bytes, std::string const & charset)
{
	...
	if (charset == "ISO-8859-5")
		return raw_bytes_to_utf8_internal(bytes, charset_vals.ISO_8859_5);
    
	if (charset == "Windows-1251" || charset == "Win-1251" || charset == "win-1251" || charset == "windows-1251")
		return raw_bytes_to_utf8_internal(bytes, charset_vals.Win_1251);	
